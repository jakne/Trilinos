...
//    3 |-1  -2  -1   4
//
template <typename ViewType, typename SC>
KOKKOS_INLINE_FUNCTION void elementStiffnessMatOnRefElementQ1(ViewType &elementMatrix)
{
...
int main(int argc, char *argv[])
{
    ...
    if (comm->getSize() > N * N) {
        cout << "Number of ranks must be a square number, since the domain is decomposed into N x N subdomains."
             << endl;
        return EXIT_FAILURE;
    }
    ...
    Teuchos::RCP<const Tpetra::Map<LO, GO, NO>> row_map = Teuchos::rcp(new Tpetra::Map<LO, GO, NO>(
        INVALID, mesh.getOwnedNodeGlobalIDs().getDeviceView(Tpetra::Access::ReadOnly), 0, comm));
    auto domain_map = row_map;
    // auto range_map = row_map;
    Teuchos::RCP<const Tpetra::Map<LO, GO, NO>> owned_plus_shared_map = Teuchos::rcp(new Tpetra::Map<LO, GO, NO>(
        INVALID, mesh.getOwnedAndGhostNodeGlobalIDs().getDeviceView(Tpetra::Access::ReadOnly), 0, comm));
#ifdef HAVE_FROSch_DEBUG
    ...
    // Build FECrsGraph.
    Teuchos::RCP<Tpetra::FECrsGraph<LO, GO, NO>> fe_graph =
        Teuchos::rcp(new Tpetra::FECrsGraph<LO, GO, NO>(row_map, owned_plus_shared_map, 16));
    ...
    Teuchos::RCP<Tpetra::FEMultiVector<SC, LO, GO, NO>> rhs;
    {
        ...
        // 4 x 4 element stiffness matrix
        Kokkos::View<SC[4][4], MeshDatabase<GO, NO>::hostType> element_matrix("element_matrix");

        // 4 x 1 element load vector
        Teuchos::Array<SC> element_rhs(4);
        ...
        for (std::size_t element_idx = 0; element_idx < mesh.getNumOwnedElements(); ++element_idx) {
            ...
            for (int element_node_idx = 0; element_node_idx < 4; ++element_node_idx) {
                ...
                for (int col_idx = 0; col_idx < 4; col_idx++) {
                    ...
                    if (    (global_column_ids[col_idx] == global_row_id) &&
                            ((global_row_id < nex + 1) || (global_row_id % (nex + 1) == 0))) {
                    ...
                    } else if ((global_row_id < nex + 1) || (global_row_id % (nex + 1) == 0) ||
                               (global_column_ids[col_idx] < nex + 1) ||
                               (global_column_ids[col_idx] % (nex + 1) == 0)) {
    ...
    Teuchos::RCP<Tpetra::FECrsMatrix<SC, LO, GO, NO>> neumann_matrices =
        FROSch::FECrsMatrix_deepCopy(fe_matrix.getConst(), fe_graph);

#ifdef HAVE_FROSch_DEBUG
    if (parameterList_debug_main->get("Row Map of Stiffness Matrix before endAssembly", false))
        FROSch::debug::printMap(
            fe_matrix->getRowMap(), "Row Map of Stiffness Matrix before endAssembly", __FILE__, __LINE__);
    if (parameterList_debug_main->get("Col Map of Stiffness Matrix before endAssembly", false))
        FROSch::debug::printMap(
            fe_matrix->getColMap(), "Col Map of Stiffness Matrix before endAssembly", __FILE__, __LINE__);
    if (parameterList_debug_main->get("Local Neumann Matrices", false))
        FROSch::debug::printTpetraCrsMatrix_local_sparse(
            fe_matrix, "Local Neumann Matrices (fe_matrix before endAssembly)", __FILE__, __LINE__);
#endif
    ...
    if (parameterList_debug_main->get("Repeated Map", false))
        FROSch::debug::printMap(
            neumann_matrices->getRowMap(), "Repeated Map = Row Map of Local Neumann Matrices", __FILE__, __LINE__);
#endif

    // Convert Tpetra::FEMultiVector to Xpetra::MultiVector.
    Teuchos::RCP<Tpetra::MultiVector<SC, LO, GO, NO>> rhs_tpetra =
        Teuchos::rcp_dynamic_cast<Tpetra::MultiVector<SC, LO, GO, NO>>(rhs);
    Teuchos::RCP<Xpetra::TpetraMultiVector<SC, LO, GO, NO>> rhs_xpetra_tpetra =
        Teuchos::rcp(new Xpetra::TpetraMultiVector<SC, LO, GO, NO>(rhs_tpetra));
    ...
    Teuchos::RCP<Tpetra::CrsMatrix<SC, LO, GO, NO>> crs_tpetra =
        Teuchos::rcp_dynamic_cast<Tpetra::CrsMatrix<SC, LO, GO, NO>>(fe_matrix);
    Teuchos::RCP<Xpetra::TpetraCrsMatrix<SC, LO, GO, NO>> crs_xpetra_tpetra =
        Teuchos::rcp(new Xpetra::TpetraCrsMatrix<SC, LO, GO, NO>(crs_tpetra));
    Teuchos::RCP<Xpetra::CrsMatrix<SC, LO, GO, NO>> crs_xpetra =
    ...
    Teuchos::RCP<Xpetra::MultiVector<SC, LO, GO, NO>> solution_xpetra =
        Xpetra::MultiVectorFactory<SC, LO, GO, NO>::Build(K->getDomainMap(), 1);
    // This will store the solution and is used for the initial vector of the iterative solver.
    solution_xpetra->putScalar(Teuchos::ScalarTraits<SC>::zero());
    ...
    Teuchos::RCP<const Thyra::LinearOpBase<SC>> K_thyra =
        Xpetra::ThyraUtils<SC, LO, GO, NO>::toThyra(crsWrapK.getCrsMatrix());
    Teuchos::RCP<Thyra::MultiVectorBase<SC>> solution_thyra = Teuchos::rcp_const_cast<Thyra::MultiVectorBase<SC>>(
        Xpetra::ThyraUtils<SC, LO, GO, NO>::toThyraMultiVector(solution_xpetra));
    Teuchos::RCP<const Thyra::MultiVectorBase<SC>> rhs_thyra =
    ...
    Teuchos::RCP<Thyra::LinearOpWithSolveFactoryBase<SC>> lowsFactory =
