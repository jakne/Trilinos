...
namespace FROSch {
    ...
    template <class SC,class LO,class GO,class NO>
    void CoarseSpace<SC,LO,GO,NO>::addSubspaceOnlyMap(ConstXMapPtr subspaceBasisMap) {
        UnassembledBasesMaps_.push_back(subspaceBasisMap);
    }

    template <class SC,class LO,class GO,class NO>
    Teuchos::RCP<Teuchos::Array<GO>> CoarseSpace<SC,LO,GO,NO>::getMapVector(int i) {
        return UnassembledBasesVecs_.at(i);
    }

    template <class SC,class LO,class GO,class NO>
    int CoarseSpace<SC,LO,GO,NO>::addSubspaceT(Teuchos::RCP<GOVec> subspaceBasisVec, // $$$
...
//        }
        ...
        UnassembledBasesVecs_.push_back(subspaceBasisVec);
//        UnassembledBasesMaps_.push_back(subspaceBasisMap);
...
//        LocalSubspacesSizes_.push_back(subspaceBasisMap->getLocalNumElements());
    ...
    template <class SC,class LO,class GO,class NO>
    int CoarseSpace<SC,LO,GO,NO>::assembleCoarseSpaceT()
    {
      // $$$        FROSCH_ASSERT(UnassembledBasesMaps_.size()>0,"FROSch::CoarseSpace: UnassembledBasesMaps_.size()==0");
        ...
        // BasisMap
//        AssembledBasisMap_ = AssembleMapsT(UnassembledBasesMaps_(),partMappings,this->MpiComm_); $$$
        Teuchos::ArrayView<Teuchos::RCP<Teuchos::Array<GO>>> cccc = UnassembledBasesVecs_();
        AssembledBasisMap_ = AssembleMapsT<LO,GO,NO>(cccc,partMappings,this->MpiComm_); //
...
//	  AssembledBasisMapUnique_ = AssembleMapsT(UnassembledBasesMapsUnique_(),partMappings,this->MpiComm_);
	  AssembledBasisMapUnique_ = AssembleMaps(UnassembledBasesMapsUnique_(),partMappings);
        ...
        if (!AssembledBasisMap_.is_null()) {
            if (AssembledBasisMap_->getGlobalNumElements()>0) { // AH 02/12/2019: Is this the right condition? Seems to work for now...
			    ...
			    std::cout << "UUU[" << this->MpiComm_->getRank() << "|" << i << "] " << AssembledBasis_->getNumVectors() << "   " << NumVectors_i << "   " << itmp << std::endl;
    ...
    int CoarseSpace<SC,LO,GO,NO>::assembleCoarseSpace()
    {
        ...
        // BasisMap
        AssembledBasisMap_ = AssembleMapsG(UnassembledBasesMaps_(),partMappings,this->MpiComm_);
