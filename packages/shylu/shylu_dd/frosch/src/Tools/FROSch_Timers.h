...
// Currently, we support no timers (0), selected timers (1), and all timers (2)
    ...
    #define FROSCH_TIMER_DETAILS 1
        ...
        #define FROSCH_TIMER_START(A,S) Teuchos::RCP<Teuchos::TimeMonitor> A = Teuchos::rcp(new Teuchos::TimeMonitor(*Teuchos::TimeMonitor::getNewTimer(std::string("FROSch: ") + std::string(S))));
    #endif

    #ifndef FROSCH_TIMER_START_LEVELID
        #define FROSCH_TIMER_START_LEVELID(A,S) Teuchos::RCP<Teuchos::TimeMonitor> A = Teuchos::rcp(new Teuchos::TimeMonitor(*Teuchos::TimeMonitor::getNewTimer(std::string("FROSch: ") + std::string(S) + " (Level " + std::to_string(this->LevelID_) + std::string(")"))));
    #endif
        ...
        #define FROSCH_TIMER_START_SOLVER(A,S) Teuchos::RCP<Teuchos::TimeMonitor> A = Teuchos::rcp(new Teuchos::TimeMonitor(*Teuchos::TimeMonitor::getNewTimer(std::string("FROSch: ") + std::string(S) + " (" + this->Description_ + std::string(")"))));
    #endif

    #ifndef FROSCH_TIMER_START_SUBDOMAINSOLVER
        #define FROSCH_TIMER_START_SUBDOMAINSOLVER(A,S) Teuchos::RCP<Teuchos::TimeMonitor> A = Teuchos::rcp(new Teuchos::TimeMonitor(*Teuchos::TimeMonitor::getNewTimer(std::string("FROSch: ") + std::string(S) + " (" + this->Description_ + std::string(")"))));
    #endif
